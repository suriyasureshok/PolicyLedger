...
*A Google Cloud–native, fallback-safe RL Policy Marketplace for Cybersecurity Decision Policies*
...
Reinforcement learning agents train cyber defense decision policies at the edge but lack trust in policy submissions. Verifying policies without retraining is computationally expensive, and no marketplace exists for RL policies due to the inability to verify claimed performance.

## 🚀 Solution

PolicyLedger learns at the edge, verifies in the cloud, remembers immutably, and reuses intelligence. Decentralized RL agents submit cyber defense policies to a marketplace where they are verified through deterministic replay, stored in a tamper-evident ledger, and made available for reuse by consumers.
...
2. **Policy Submission**: Agents submit trained defense policies with claimed performance scores to a submission layer (Firebase REST API or local queue).

3. **Cloud Verification**: Vertex AI custom jobs verify policies by deterministically replaying them in the same simulated environment and recomputing defense scores. Invalid claims are rejected.
...
5. **Marketplace Ranking**: Cloud Functions trigger on new ledger entries and rank policies by verified performance, updating the best policy reference.

6. **Policy Reuse**: Consumers fetch the best policy from the marketplace and deploy it immediately without retraining, demonstrating intelligent reuse.

7. **Explainability**: Human-readable explanations of why winning defense policies outperform baselines, generated by fallback templates or Gemini AI.
...
- **Vertex AI**: Custom jobs for deterministic policy verification and performance recomputation
...
│   │   ├── cyber_env.py       # Cyber defense decision simulation
...
**CyberDefenseEnv**: A deterministic decision-level cyber defense simulation where agents learn to respond to attack indicators. The environment uses seeded randomness to ensure reproducibility — same seed produces identical trajectories.

⚠️ **IMPORTANT**: This is a SIMULATED environment for demonstrating RL policy verification and reuse. It is NOT a real cybersecurity system and does NOT provide actual protection.

**Environment Details**:
- **State Space**: Attack severity, attack type, system health, alert confidence, time under attack
- **Action Space**: IGNORE, MONITOR, RATE_LIMIT, BLOCK_IP, ISOLATE_SERVICE
- **Reward Function**: Balances damage prevention vs. operational costs (false positives, service disruption)
